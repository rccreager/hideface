...
output_dir = 'data/test_performance'
epsilon_start_value = 8
...
for recognizer_name, _ in recognizer_dict.items():
    for img_path in img_paths:
        ...
        truth_iou_no_noise = image_labels.true_box_list[0].iou(image_labels.found_box_dict[recognizer_name][0])
        ...
        while (epsilon < max_epsilon_value):
            ...
            truth_iou_noise = 0 if (len(noisy_image_labels.found_box_dict[recognizer_name]) == 0) else image_labels.true_box_list[0].iou(noisy_image_labels.found_box_dict[recognizer_name][0])
            if (len(noisy_image_labels.found_box_dict[recognizer_name]) == 0):
                ...
                performance_list.append({'img_num':img_num,'epsilon':epsilon,'img_shape':image_labels.img_shape,  
                ...
                break
            if (len(noisy_image_labels.found_box_dict[recognizer_name]) == 1 and truth_iou_noise < iou_cutoff_value):
        ...
        else: print('Noise attack failed'); tunnel_dict['fail_count'] += 1
print(performance_list)
with open(output_dir + '/' + performance_list_csv, 'w', encoding='utf8', newline='') as output_file:
    fc = csv.DictWriter(output_file,fieldnames=performance_list[0].keys(),quotechar = "'")
    fc.writeheader()
    fc.writerows(performance_list)
print(tunnel_dict)
with open(output_dir + '/' + tunnel_csv, 'w', encoding='utf8', newline='') as output_file:
    fc = csv.DictWriter(output_file,fieldnames=tunnel_dict.keys(),quotechar = "'")
    fc.writeheader()
