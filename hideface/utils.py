...
def draw_and_return_noisy_image_labels(original_img_path, max_noise_epsilon, output_dir):
    ...
    return ImageLabels(noisy_file_path,kwargs={'noise_epsilon':max_noise_epsilon})

def draw_labeled_image(image_labels, output_dir, attack_str=""):
    """
    ...
    Args:
        ...
        attack_str: an optional string describing the attack applied to this image
    """
    if (not os.path.exists(output_dir)): os.makedirs(output_dir)
    ...
    if (len(image_labels.found_box_dict) == 0): 
        ...
        if (len(image_labels.true_box_list) == 0): name_str += '_no_truth'
        if (attack_str != ""): name_str = name_str + '_' + attack_str + '_'
        io.imsave(output_dir + '/boxed_'+name_str+file_num+'.jpg', image)
    for recognizer_name, found_box_list in image_labels.found_box_dict.items():
        tools.draw_boxes(image, found_box_list, (255,0,0))
        name_str = ''
        if (attack_str != ""): name_str = name_str + '_' + attack_str + '_'
        io.imsave(output_dir + '/boxed_'+ name_str + recognizer_name+'_'+file_num+'.jpg', image)
