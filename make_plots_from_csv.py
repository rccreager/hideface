...
def draw_histo(df, col_name, x_start, bin_width, n_bins, title, output_dir, output_filename):
    ...
    """
    plt = df[col_name].plot.hist(bins=[bin_width*(x+0.5)+x_start for x in np.arange(0,n_bins)], edgecolor='black', color='#88d498')
    plt.set_title(title)
...
if __name__ == "__main__":
    ...
    with open (performance_csv) as csv_file:
        df = pd.read_csv(csv_file, dtype={'img_num':str,'epsilon':int,'img_size': int, 'true_box_size':int,'truth_iou_no_noise':float,'truth_iou_noise':float,'ssim':float})  
        print(df.head(10))
        df_attacks = df[df['epsilon'] >= 0] # get rid of rows from failed attacks  
    
        draw_histo(df_attacks, 'epsilon', -16, 16, 16, 'Epsilon Additive Noise Value to Beat HoG', output_dir, 'eps_hist.png')
        draw_histo(df_attacks, 'img_size', 400000, 100000, 21, 'Image Size (Width x Height)', output_dir, 'img_size_hist.png') 
        draw_histo(df_attacks, 'true_box_size', -15000, 15000, 25, 'True Face Box Size (Width x Height)', output_dir, 'true_box_size_hist.png') 
        draw_histo(df_attacks, 'truth_iou_no_noise', -0.05, 0.05, 21, 'IoU between Found and True Box Prior to Attack', output_dir, 'truth_iou_no_noise.png')
        draw_histo(df_attacks, 'truth_iou_noise', -0.05, 0.05, 21, 'IoU between Found and True Box After Noise Attack', output_dir, 'truth_iou_noise.png')
        draw_histo(df_attacks, 'ssim', -0.05, 0.05, 21, 'Structural Similarity', output_dir, 'ssim.png')
