...
for recognizer_name, _ in recognizer_dict.items():
    for img_path in img_paths:
        ...
        noisy_image_labels = attacks.draw_and_return_noisy_image_labels(img_path, epsilon, output_dir).add_all_labels(truth_file, recognizer_dict)
        while (epsilon < max_epsilon_value):
            ...
            if (len(noisy_image_labels.found_box_dict[recognizer_name]) == 1):
                if (image_labels.true_box_list[0].iou(noisy_image_labels.found_box_dict[recognizer_name][0]) < iou_cutoff_value):
